...
@model("meta_sasrec_vqvae", MetaType.ModelBuilder)
class SasRec(nn.Module):
    def __init__(self, model_conf):
        ...
        self._meta_classifier_param_list = common.HyperNetwork_FC_VQVAE(
            model_conf.id_dimension,
    ...
    def forward(self, features, fig1=False):
        ...
        user_embedding, vae_loss = self._meta_classifier_param_list(user_state, hist_embed,
                                         user_state.size()[0],
                                         seq_length)
        if fig1:
        ...
        return torch.sum(user_embedding * target_embed, dim=1, keepdim=True), target_embed, vae_loss
