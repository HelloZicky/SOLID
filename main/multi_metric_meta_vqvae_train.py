...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None):
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        logits, target_embed, vae_loss = model_obj({
            key: value.to(device)
...
def train(train_dataset, model_obj, device, args, pred_dataloader):
    ...
    for epoch in range(1, args.max_epoch + 1):
        for step, batch_data in enumerate(train_dataset, 1):
            ...
            bce_loss = criterion(logits, batch_data[consts.FIELD_LABEL].view(-1, 1).to(device))
            loss = bce_loss + 0.1 * vae_loss
            # print("=" * 50, "in train() ", "=" * 50)
            # print("BCEloss = ", loss)
            # print("total vae loss = ", vae_loss)
            ...
            # vqvae_enc.backward(model_obj._meta_classifier_param_list.vqvae.grad_for_encoder)
            optimizer.step()
